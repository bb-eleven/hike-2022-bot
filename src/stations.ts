...
export const loadStationConfigs = async () => {
  ...
  const stationConfigsVals = (await sheets.spreadsheets.values.get(createSheetsRequest('Stations!B2:D8'))).data.values;
  
  Object.keys(STATION_CELLS).forEach((name, index) => {
    const scores: number[] = stationConfigsVals?.[index].map(scoreStr => Number(scoreStr)) ?? [20, 50];
